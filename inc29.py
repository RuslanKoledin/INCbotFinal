...
BROADCAST_GROUPS = [
    ...
    {"chat_id": -1002431919774, "thread_id": 2},  #Группа с темой "Инциденты" ДКОиОК Инцидент менеджмент
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "Инцидент:" in text and "Время выявления инцидента:" in text:
        ...
        if incident_id in incidents:
            ...
            await safe_send(context.bot, {"chat_id": chat_id}, f"Инцидент '{incident_id}' уже существует.")
            return
        ...
        if ("Средний" not in text) and ("Низкий" not in text):
            job_50 = scheduler.add_job(notify_50_minutes, 'date', run_date=now + datetime.timedelta(minutes=50),
                                       args=[application, chat_id, incident_id, loop], id=f"{incident_id}_50")
            job_60 = scheduler.add_job(notify_60_minutes, 'date', run_date=now + datetime.timedelta(minutes=60),
                                       args=[application, chat_id, incident_id, loop], id=f"{incident_id}_60")
    ...
    if any(word in text for word in priority_words):
        ...
        if ("поднят до" in text or "повышен до" in text) and ("высокий" in text.lower() or "критичный" in text.lower()):
            print(f"Приоритет инцидента повышен.")
            await safe_send(context.bot, {"chat_id": chat_id}, f"Приоритет инцидента '{key}' повышен.")
            ...
            job_50 = scheduler.add_job(notify_50_minutes, 'date', run_date=datetime.datetime.now(tz=ZoneInfo("Asia/Bishkek")) + remain_50,
                                       args=[application, chat_id, incident_id, loop], id=f"{incident_id}_50")
            job_60 = scheduler.add_job(notify_60_minutes, 'date', run_date=datetime.datetime.now(tz=ZoneInfo("Asia/Bishkek")) + remain_60,
                                       args=[application, chat_id, incident_id, loop], id=f"{incident_id}_60")
            ...
            await safe_send(context.bot, {"chat_id": chat_id}, f"Приоритет инцидента '{key}' понижен.")
            for job_id in incident["jobs"]:
                try:
                    scheduler.remove_job(job_id)
                except Exception as e:
...
def notify_3_hours_later(app, chat_id, incident_id, loop):
    if incident_id not in incidents:
        return
    asyncio.run_coroutine_threadsafe(
        app.bot.send_message(chat_id=chat_id, text="Прошло 3 часа! Проверьте статус.@Elturan, @Ruslank1111"), loop
    )
...
def restore_jobs(incidents):
    ...
    for incident_id, incident in incidents.items():
        ...
        job_50 = scheduler.add_job(notify_50_minutes, 'date', run_date=now + remain_50,
                                   args=[application, incident["chat_id"], incident_id, loop], id=f"{incident_id}_50")
        job_60 = scheduler.add_job(notify_60_minutes, 'date', run_date=now + remain_60,
                                   args=[application, incident["chat_id"], incident_id, loop], id=f"{incident_id}_60")
